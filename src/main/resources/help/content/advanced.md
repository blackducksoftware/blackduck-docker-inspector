...
the components generated by ${solution_name} contains only components from your application layers.
...
to run ${solution_name} without access to the internet. To download the Air Gap archive, run the command:
...
(and therefore ${solution_name}) for your Docker registry can be found at:
...
and you are calling ${solution_name} from Detect, you will need to put the properties
